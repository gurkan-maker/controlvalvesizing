...
# ========================
PIPE_SCHEDULE_DB = {
    # Format: (nominal_size, schedule): (od, wall_thickness, id)
    # Values in inches
    (0.5, "40"): (0.840, 0.109, 0.622),
    (0.5, "80"): (0.840, 0.147, 0.546),
    (0.5, "160"): (0.840, 0.187, 0.466),
    
    (1, "40"): (1.315, 0.133, 1.049),
    (1, "80"): (1.315, 0.179, 0.957),
    (1, "160"): (1.315, 0.250, 0.815),
    
    (1.5, "40"): (1.900, 0.145, 1.610),
    (1.5, "80"): (1.900, 0.200, 1.500),
    (1.5, "160"): (1.900, 0.281, 1.338),
    
    (2, "40"): (2.375, 0.154, 2.067),
    (2, "80"): (2.375, 0.218, 1.939),
    (2, "160"): (2.375, 0.343, 1.689),
    
    (3, "40"): (3.500, 0.216, 3.068),
    (3, "80"): (3.500, 0.300, 2.900),
    (3, "160"): (3.500, 0.438, 2.624),
    
    (4, "40"): (4.500, 0.237, 4.026),
    (4, "80"): (4.500, 0.337, 3.826),
    (4, "160"): (4.500, 0.531, 3.438),
    
    (6, "40"): (6.625, 0.280, 6.065),
    (6, "80"): (6.625, 0.432, 5.761),
    (6, "160"): (6.625, 0.719, 5.187),
    
    (8, "40"): (8.625, 0.322, 7.981),
    (8, "80"): (8.625, 0.500, 7.625),
    (8, "160"): (8.625, 0.906, 6.813),
    
    (10, "40"): (10.750, 0.365, 10.020),
    (10, "80"): (10.750, 0.500, 9.750),
    (10, "160"): (10.750, 1.125, 8.500),
    
    (12, "40"): (12.750, 0.375, 12.000),
    (12, "80"): (12.750, 0.500, 11.750),
    (12, "160"): (12.750, 1.312, 10.126),
    
    (14, "STD"): (14.000, 0.375, 13.250),
    (14, "XS"): (14.000, 0.500, 13.000),
    (14, "XXS"): (14.000, 1.000, 12.000),
    
    (16, "STD"): (16.000, 0.375, 15.250),
    (16, "XS"): (16.000, 0.500, 15.000),
    (16, "XXS"): (16.000, 1.000, 14.000),
    
    (18, "STD"): (18.000, 0.375, 17.250),
    (18, "XS"): (18.000, 0.500, 17.000),
    (18, "XXS"): (18.000, 1.000, 16.000),
    
    (20, "STD"): (20.000, 0.375, 19.250),
    (20, "XS"): (20.000, 0.500, 19.000),
    (20, "XXS"): (20.000, 1.000, 18.000),
    
    (24, "STD"): (24.000, 0.375, 23.250),
    (24, "XS"): (24.000, 0.500, 23.000),
    (24, "XXS"): (24.000, 1.000, 22.000),
    
    (30, "STD"): (30.000, 0.375, 29.250),
    (30, "XS"): (30.000, 0.500, 29.000),
    (30, "XXS"): (30.000, 1.000, 28.000),
}

def get_pipe_dimensions(nominal_size: float, schedule: str) -> tuple:
    """Get pipe dimensions for given nominal size and schedule"""
    key = (nominal_size, schedule)
    if key in PIPE_SCHEDULE_DB:
        return PIPE_SCHEDULE_DB[key]
    # Fallback: assume nominal size as ID
    return (nominal_size, 0, nominal_size)
...
def evaluate_valve_for_scenario(valve, scenario):
    # Determine pipe dimensions based on selection
    if scenario.get("use_valve_size", True):
        pipe_nominal = valve.diameter
        pipe_schedule = scenario.get("pipe_schedule", "40")
        _, _, pipe_id = get_pipe_dimensions(pipe_nominal, pipe_schedule)
    else:
        pipe_nominal = scenario["pipe_nominal"]
        pipe_schedule = scenario["pipe_schedule"]
        _, _, pipe_id = get_pipe_dimensions(pipe_nominal, pipe_schedule)
        
    valve_d = valve.diameter
    cv_100 = valve.get_cv_at_opening(100)
    
    # Use pipe_id (actual inside diameter) in calculations
    fp = calculate_piping_factor_fp(valve_d, pipe_id, cv_100)
...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        ...
        if not use_valve_size:
            pipe_nominal = st.number_input(
                "Pipe Nominal Size (inch)", 
                min_value=0.1, 
                max_value=100.0, 
                value=scenario_data.get("pipe_nominal", scenario_data["pipe_d"]), 
                step=0.1,
                key=f"pipe_nominal_{scenario_num}"
            )
            
            # Get available schedules for this pipe size
            available_schedules = sorted({s for (size, s) in PIPE_SCHEDULE_DB.keys() if size == pipe_nominal})
            default_schedule = available_schedules[0] if available_schedules else "40"
            
            pipe_schedule = st.selectbox(
                "Pipe Schedule",
                options=available_schedules,
                index=available_schedules.index(scenario_data.get("pipe_schedule", default_schedule)) if available_schedules else default_schedule,
                key=f"pipe_schedule_{scenario_num}"
            )
    ...
    return {
        ...
        "pipe_id": pipe_id,  # Actual inside diameter to use in calculations
    }
